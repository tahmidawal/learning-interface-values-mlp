...
def generate_low_k_case(data_gen: DataGeneratorV2, k1: float, k2: float):
    """Generate a case with specific k values."""
    coef = np.random.uniform(0.1, 0.2)  # Keep the same coefficient range as training
    
    # Generate solution with specified frequencies
...
def main():
    try:
        ...
        results_dir = 'low_k_test_results'
        ...
        for k in k_values:
            ...
            theta, f, u_exact = generate_low_k_case(data_gen, k, k)
        ...
        print("\nProcessing complete! Check the low_k_test_results directory for output plots.")
