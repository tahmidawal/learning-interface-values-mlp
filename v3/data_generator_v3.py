...
class DataGeneratorV3:
    ...
    Uses constant theta=1 and low frequency k values.
    ...
    def __init__(self, nx: int = 40, ny: int = 40, n_subdomains: Tuple[int, int] = (2, 2),
        ...
        # Wavenumbers for solution generation (low frequency)
    ...
    def generate_random_problem(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Generate random problem with numerical solution.
        All values are scaled to be between -1 and 1.
        
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: theta, f, and numerical solution
        """
        # Select random k values
        k1 = np.random.choice(self.k_values)
        k2 = np.random.choice(self.k_values)
        
        # Random coefficient (small to keep solution well-behaved)
        ...
        bc_left = lambda y: coef * np.sin(k1 * 2 * np.pi * 0) * np.sin(k2 * 2 * np.pi * y)
        bc_right = lambda y: coef * np.sin(k1 * 2 * np.pi * 1) * np.sin(k2 * 2 * np.pi * y)
        bc_bottom = lambda x: coef * np.sin(k1 * 2 * np.pi * x) * np.sin(k2 * 2 * np.pi * 0)
        bc_top = lambda x: coef * np.sin(k1 * 2 * np.pi * x) * np.sin(k2 * 2 * np.pi * 1)
        
        # Solve PDE numerically
        u_numerical, _ = self.solver.solve_full_domain(theta, -f, bc_left, bc_right, bc_bottom, bc_top)
    ...
    def generate_boundary_conditions(self, u: np.ndarray) -> Dict[str, Callable]:
        """
        Generate boundary conditions from the numerical solution.
